...
## Code Evidence
```java
    51 | 	@PostMapping(value = "{sessionId}", consumes = "multipart/form-data")
    52 | 	public void importPublicKeys(
>>>   53 | 			@PathVariable
    54 | 			final String sessionId,
    55 | 			final MultipartRequest multipartRequest) {
```
...
## Impact Template
Endpoint [extract endpoint] at e-voting\tools\direct-trust-tool\direct-trust-tool-backend\src\main\java\ch\post\it\evoting\directtrusttool\backend\api\v1\PublicKeysSharingController.java:53 accepts RequestBody/PathVariable without explicit @Valid. Requires concrete exploit and reachable attack path.
