...
## Code Evidence
```java
    53 | 	@GetMapping(value = "{sessionId}/selected", produces = "application/json")
    54 | 	public ImmutableSet<Alias> getSelectedKeystore(
>>>   55 | 			@PathVariable
    56 | 			final String sessionId) {
    57 | 		validateSessionId(sessionId);
```
...
## Impact Template
Endpoint [extract endpoint] at e-voting\tools\direct-trust-tool\direct-trust-tool-backend\src\main\java\ch\post\it\evoting\directtrusttool\backend\api\v1\SessionController.java:55 accepts RequestBody/PathVariable without explicit @Valid. Requires concrete exploit and reachable attack path.
