...
## Code Evidence
```java
    85 | 	@PutMapping("votingcards/{" + VOTING_CARD_ID + "}/block")
    86 | 	public void blockVotingCard(
>>>   87 | 			@PathVariable(VOTING_CARD_ID)
    88 | 			final String votingCardId) {
    89 | 		validateUUID(votingCardId);
```
...
## Impact Template
Endpoint [extract endpoint] at e-voting\voting-server\src\main\java\ch\post\it\evoting\votingserver\process\votingcardmanagement\VotingCardManagerController.java:87 accepts RequestBody/PathVariable without explicit @Valid. Requires concrete exploit and reachable attack path.
