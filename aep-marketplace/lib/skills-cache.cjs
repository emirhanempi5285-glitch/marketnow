 ...
 * Estrategia:
 *   1. Si cache válido → devolver cache (instantáneo)
 *   2. Si hay loading en progreso → esperar (evita thundering herd)
 *   3. Sino → cargar desde archivo (sync) o fetch (fallback)
 */
async function getSkills() {
  ...
  // Evitar thundering herd: si ya hay una carga en progreso, esperar
  if (_loadingPromise) {
...
async function _loadSkills() {
  ...
  // Estrategia 1: leer archivo desde sistema (build-time embed)
  // En Vercel, los archivos en /public se sirven desde dist/, pero también
  // podemos leerlos directamente del filesystem del serverless.
  const candidates = [
  ...
  // Solo se usa si los archivos no existen (no debería pasar en producción)
...
/**
 * Cache de free-skills.json (lista curada, ~50KB).
 */
 ...
 * Merguea flags: si está en free list, marca price=0.
 */
...
/**
 * Stats del cache (para /health o /trust).
 */
function getStats() {
  return {
    cached: !!_cache,
...
/**
 * Invalida el cache (para tests o para forzar reload).
 */
function invalidate() {
  _cache = null;
  _freeCache = null;
